...
const CommentHeader = ({
  ...
  const handleUnlike = (e) => {
    e.preventDefault();
    setLikeStatus(false);
    setCurrentBtn('single-unliking');
    setAddLike(false);
    return unlikeAction(postID);
  };

  const likes = results && typeof results.totalLikes !== 'undefined' ?
    results.totalLikes
    : data ? data.totalLikes : results.likesNo;
  const changeLikes = currentBtn === 'single-liking'
    ? likes !== 0
      ? likes
      : likes + 1
    : currentBtn === 'single-unliking'
      ? likes !== 0
        ? likes - 1
        : likes
      : likes;
  return (
    <div className="profile-modal-comment-body-footer px-2 pt-2 pb-2">
      {currentBtn === 'single-liking'
        || (results.isLiked && currentBtn !== 'single-unliking')
        || likeStatus ? (
          <span
      ...
      <span
        ...
        style={{ fontSize: 13 }}
      >
        {generatePostedAt(results.createdAt)}
      </span>
    </div>
  );
};
